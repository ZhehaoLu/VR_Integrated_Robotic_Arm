...
Vector<float, 6> wall_vec() {
  ...
  Matrix4f Tsp{
      {puck_transformationmatrix[0][0], puck_transformationmatrix[1][0],
       puck_transformationmatrix[2][0], puck_transformationmatrix[3][0]},
      {puck_transformationmatrix[0][1], puck_transformationmatrix[1][1],
       puck_transformationmatrix[2][1], puck_transformationmatrix[3][1]},
      {puck_transformationmatrix[0][2], puck_transformationmatrix[1][2],
       puck_transformationmatrix[2][2], puck_transformationmatrix[3][2]},
      {puck_transformationmatrix[0][3], puck_transformationmatrix[1][3],
       puck_transformationmatrix[2][3], puck_transformationmatrix[3][3]}};
  // Matrix4f Tpb
  // {{0.,-1.,0.,-0.05},{-0.8,0.,-0.5,-.3},{0.5,0.,-0.8,0.19},{0.,0.,0.,1.}};
  Matrix4f Tpb{{-1., 0., 0., -0.15},
               {.0, 0., -1., -.22},
               {0., 1., 0., 0.12},
               {0., 0., 0., 1.}};
  Matrix4f Tsw{
      {wall_transformationmatrix[0][0], wall_transformationmatrix[1][0],
       wall_transformationmatrix[2][0], wall_transformationmatrix[3][0]},
      {wall_transformationmatrix[0][1], wall_transformationmatrix[1][1],
       wall_transformationmatrix[2][1], wall_transformationmatrix[3][1]},
      {wall_transformationmatrix[0][2], wall_transformationmatrix[1][2],
       wall_transformationmatrix[2][2], wall_transformationmatrix[3][2]},
      {wall_transformationmatrix[0][3], wall_transformationmatrix[1][3],
       wall_transformationmatrix[2][3], wall_transformationmatrix[3][3]}};
  Matrix4f Tsb = Tsp * Tpb;
  Matrix4f Tbw = Tsb.inverse() * Tsw;
  // Vector<float, 6> wall {{Tbw(0,1), Tbw(1,1), Tbw(2,1), Tbw(0,3),
  // Tbw(1,3),Tbw(2,3)}};
  Vector<float, 6> wall{
      {Tbw(0, 2), Tbw(1, 2), Tbw(2, 2), Tbw(0, 3), Tbw(1, 3), Tbw(2, 3)}};
  return wall;
}
Vector<float, 4> theta_conv(Vector3f GR, float Theta_to_pos,
                            Vector<float, 4> motor_pos) {
...
Vector<float, 3> accel_ee(Vector<float, 4> e_e1, Vector<float, 4> e_e2,
                          Vector<float, 4> e_e3) {
...
Vector<float, 3> jacobian_torque(VectorXf trig_matrix, float a,
                                 Vector<float, 3> F, Vector<float, 3> GR) {
  // Vector<float,7> trig_matrix {{s1, s2, s3, c1,c2,c3, time}};
  float J11, J12, J13, J21, J22, J23, J31, J32, J33;
  J11 = -a * trig_matrix(0) * trig_matrix(5);
  J12 = a * trig_matrix(1) * trig_matrix(5);
  J13 = -a * trig_matrix(3) * trig_matrix(2) +
        a * trig_matrix(4) * trig_matrix(2);
  J21 = a * trig_matrix(3);
  J22 = -a * trig_matrix(4);
  J23 = 0;
  ...
  J33 =
      a * trig_matrix(3) * trig_matrix(5) - a * trig_matrix(4) * trig_matrix(5);
...
Vector<float, 3> whiteboard(Vector<float, 3> GR, float k, float c, float a,
                            float theta_to_pos, Vector<float, 6> wall,
                            Vector<float, 4> motor_pos) {
...
void loop() {
  if (Serial5.available() > 0) {
    ...
    // calculate sines and cosines of capstan radians (converts motor rads to
    // capstan too)
    ...
    if (incoming_string.substring(0, 1) == 'a') {
      ...
      controller_transformation.toCharArray(controller_temp,
                                            controller_transformation.length());
    ...
    // run whiteboard
    Vector<float, 3> Tau =
        whiteboard(GR, k, c, a, Theta_to_pos, wall, motor_pos);
